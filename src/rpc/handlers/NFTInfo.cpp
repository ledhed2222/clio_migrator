...
doNFTInfo(Context const& context)
{
    ...
    catch (Backend::UnexpectedDataError const& e)
    {
        return Status{
            RippledError::rpcDB_DESERIALIZATION,
            "NFT has no URI recorded. Try again in a moment and if this error "
            "still persists open an issue as this is unexpected"};
    }
    if (!dbResponse)
