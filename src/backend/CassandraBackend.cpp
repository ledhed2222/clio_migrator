    ...
    boost::asio::yield_context& yield) const
{
    ...
    if (!uriResponse)
        throw UnexpectedDataError(
            "NFT " + ripple::strHex(tokenID) + " has no URI recorded");
...
CassandraBackend::open(bool readOnly)
{
    ...
    std::string const tablePrefix = this->tablePrefix();
    ...
    while (!setupSessionAndTable)
    {
        ...
        query.str("");
        query << "SELECT * FROM " << tablePrefix << "clio_migrations"
              << " LIMIT 1";
        if (!executeSimpleStatement(query.str()))
            continue;
    ...
    while (!setupPreparedStatements)
    {
        ...
        query.str("");
        query << "SELECT migration FROM " << tablePrefix
              << "clio_migrations WHERE migration = ? LIMIT 1";
        if (!selectClioMigration_.prepareStatement(query, session_.get()))
            continue;

        query.str("");
        query << "INSERT INTO " << tablePrefix << "clio_migrations "
              << " (migration) VALUES(?)";
        if (!insertClioMigration_.prepareStatement(query, session_.get()))
            continue;
