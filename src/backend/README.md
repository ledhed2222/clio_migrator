...
#### `clio_migrations`
```
CREATE TABLE clio.clio_migrations (
	migration varchar PRIMARY KEY				# The unique name of a succesfully-run migration
)
```
This table is internal to clio itself. It is used to note that a specific data
migration has completed succesfully. The table is called `clio_migrations` as
opposed to `migrations` to indicate that the data it holds is entirely
internal to the clio application.
...
```
This table indexes NFT IDs with their owner at a given ledger. So `SELECT *
FROM nf_tokens WHERE token_id = N AND seq <= Y ORDER BY seq DESC LIMIT 1` will
give you the owner of token N at ledger Y and whether it was burned. The
`owner` field if the token is burned will indicate the person who owned the
token at the time it was burned. It does not indicate the person who burned
the token, necessarily.
...
not useful to know all the NFTs with a taxon while excluding issuer, since the
...
version to rippled, but just in case we can handle that.
...
same reasons and serves the analogous purpose here.
