...
public:
    ...
    void
    bindNextString(std::string const& data)
    {
        if (!statement_)
            throw std::runtime_error(
                "CassandraStatement::bindNextString - statement_ is null");
        CassError rc = cass_statement_bind_string(
            statement_, curBindingIndex_, data.c_str());
        if (rc != CASS_OK)
        {
            std::stringstream ss;
            ss << "Error binding string to statement: " << rc << ", "
               << cass_error_desc(rc);
            log_.error() << __func__ << " : " << ss.str();
            throw std::runtime_error(ss.str());
        }
        curBindingIndex_++;
    }
...
private:
    ...
    CassandraPreparedStatement selectClioMigration_;
    CassandraPreparedStatement insertClioMigration_;
